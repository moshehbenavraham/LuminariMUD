#100
Obj Command 100 - portal to Midgen~
1 c 7
en~
...
if %actor.is_pc%
  if %actor.level% <= 3
    ...
    bow
    wait 1 sec
say May I suggest you visit one of the newbie areas?  Type 'help teleporter' to learn how to get around fast!
say Also I can help you PRACtice, TRAIN and BOOST your stats...
  end
  if %actor.level% >= 30
    wait 1 sec
    bow %actor.name%
...
if %actor.hitp% > 0
  ...
  %send% %actor% You are to wounded to do that!
  %echoaround% %actor% %actor.name% tries to recall, but fails.
  halt
end
~
#177
