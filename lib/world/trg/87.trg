...
if !%arg% && !%command%
  wait 2s
  %echo%  @g[ Welcome to Zone 87, by Meyekul.  Type '@oEnter Zone@n@g' to begin. ]@n
elseif %cmd.mudcommand% == Enter && zone /= %arg%
  wait 1s
  %echo% @g[ Now Entering Zone 87... ]@n
  wait 1s
  %teleport% %actor% 8701
  %force% %actor% look
else
...
~
#8702
Large Fish (8702) chase Small Fish (8701)~
0 h 100
~
if %actor.vnum% == 8701
...
if (%amount% >= 200)
  ...
  %door% 8792 n flags a
  %door% 8792 n room 8794
  %door% 8792 n key 8707
  %door% 8794 s flags a
  %door% 8794 s room 8792
  %door% 8794 s key 8707
  wait 2s
...
~
#8710
Glumgold (8711) Talks to Polly~
0 b 25
~
say Polly wanna cracker?
~
#8711
Glumgold (8711) Taunts Players~
0 h 100
~
eval taunt %random.3%
switch %taunt%
...
~
#8712
Near Death Trap Davy Jones' Locker - 8786~
2 g 100
~
...
~
#8715
Bucket O' Tar (8715) Cools after 10 Minutes~
1 g 100
~
wait 300s
...
~
#8716
Bucket O' Tar (8715) Spills when Manipulated~
1 ghjl 50
~
...
~
#8717
Bucket O' Tar - Purge Clump (8717) After 5 Minutes~
1 n 100
~
wait 300s
%purge% %self%
~
#8719
Load Meat when Crab (8719) Dies~
0 f 100
~
%load% obj 8719
~
#8720
Life Boat (8720) Dispenser~
1 g 100
~
return 0
%load% obj 8701
...
~
#8775
Load/Purge Exits after Leave (8775)~
2 q 100
~
if (%direction% == down)
  %door% 8773 up room 8775
  %door% 8773 up description You can still reach the ladder if you wish to board the ship again.
  set loop 0
  set person 1
  while (%loop% < 60)&&(%people.8773%||%people.8775%)
   wait 1s
   eval loop (%loop% + 1)
  done
  %at% 8773 %echo% It is no longer possible to board the ship from here.
  %door% 8773 up purge
end
~
#8780
Vermin Flees on Entrance (8780)~
2 g 50
~
  wait 1s
...
~
#8793
99 Bottles of Beer (Drunken Pirate: 8709)~
0 b 10
~
set beers 99
emote clears his throat.
